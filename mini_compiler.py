...
class MiniCompiler:
    def __init__(self, source, env):
        ...
        # Token: ID/angka/desimal/+,*,^,(,),-,/, kemudian '?' sebagai sentinel
        self._tokens = iter(
    ...
    def factor(self):
        ...
        elif token and token.isalpha() or (token and '_' in token) or (token and token.isidentifier()):
            # pada regex ini ID memakai [a-zA-Z_]\w*
    ...
    # Operator '^' punya prioritas lebih tinggi daripada '*' dan '/'
    # (di sini diasumsikan left-associative seperti term(), sesuai style yang diberikan)
    def power(self):
        node = self.factor()
        while self._current == '^':
            op = self._current
    ...
    def generate_tac(self, node):
        if isinstance(node, Num):
            # konsisten dengan contoh output: bilangan jadi string
